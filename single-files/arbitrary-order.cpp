...
struct remapper {
    ...
    template<class... Ls> struct leaf {
      auto operator()(Fun fun, Ls... ls) const {
        return fun(std::get<Ts>(std::tuple<Ls...>{FWD(ls)...})...);
      }
    };
...
auto make_arbitrary_order(R(*fun)(Args...)) {
  return [fun](auto&&... args) { return remapper<decltype(fun), Args...>{}(fun, FWD(args)...); };
}

void hello(const bool& b, int i, float&& f, const char* s) {
  std::cout << "hello(" << std::boolalpha << b << ", " << i << ", " << f << "(" << &f << ")" << ", " << s << ")" << std::endl;
}
...
int main() {
  ...
  world(false, 123, "test", std::move(f));
  world(123, "test", std::move(f), (void*)nullptr);
  world("test", 123U, (void*)nullptr, std::move(f));
}
